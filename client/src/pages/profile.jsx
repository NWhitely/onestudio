...
import CalendarSetup from "./calendar-setup"; // Import the CalendarSetup component
...
function Profile() {
  ...
  const [isInstructor, setIsInstructor] = useState(false); // Check if the user is an instructor
  const [googleCalendarConnected, setGoogleCalendarConnected] = useState(false); // Check Google Calendar connection
  ...
  useEffect(() => {
    ...
    if (userInfo) {
      ...
      // Check if the user is an instructor and Google Calendar is connected
      setIsInstructor(userInfo.role === "instructor");
      setGoogleCalendarConnected(userInfo.googleCalendarConnected);
    }
  }, [userInfo]);
      ...
      {isLoaded && (
        <div className="flex flex-col items-center justify-start min-h-[80vh] gap-3">
          ...
          <nav className="flex space-x-4 my-4">
            ...
            {isInstructor && (
              <button
                ...
                className={`${
                  activeTab === "calendar" ? "font-bold text-blue-500" : ""
                }`}
              >
                Calendar Setup
              </button>
          ...
          <div className="w-full">
            ...
            {activeTab === "calendar" && isInstructor && (
              <CalendarSetup googleCalendarConnected={googleCalendarConnected} />
            )}
          </div>
        </div>
      )}
